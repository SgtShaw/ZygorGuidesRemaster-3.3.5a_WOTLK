...
Name: Astrolabe
Revision: $Rev: 105 $
$Date: 2009-04-15 05:21:53 +0200 (┼Ür, 15 kwi 2009) $
...
-- protect against virtual continents
local function safe_SetMapZoom(c,z)
	if c>=0 and c<=4 then SetMapZoom(c,z) end
end
...
function Astrolabe:GetUnitPosition( unit, noMapChange )
	...
	if ( x <= 0 and y <= 0 ) then
		...
		local lastCont, lastZone = GetCurrentMapContinentAndZone()
		SetMapToCurrentZone();
		...
		if ( C ~= lastCont or Z ~= lastZone ) then
			safe_SetMapZoom(lastCont, lastZone); -- set map zoom back to what it was before
		end
		return C, Z, x, y;
	end
	local c,z = GetCurrentMapContinentAndZone()
...
function Astrolabe:GetCurrentPlayerPosition()
	...
	if ( x <= 0 and y <= 0 ) then
		...
		local lastCont, lastZone = GetCurrentMapContinentAndZone()
		if lastCont==-1 and lastZone==0 and GetMapInfo()=="ScarletEnclave" then
			lastCont=5
			lastZone=1
		end
		SetMapToCurrentZone();
...
local function activate( newInstance, oldInstance )
	if ( oldInstance ) then -- this is an upgrade activate
		...
		newInstance.ContinentList[5] = {"ScarletEnclave"}
	end
...
WorldMapSize = {
	...
	{ -- [5]
		...
		height = 27149.795290881,
		width = 40741.175327834,
		xOffset = 19242.31220836664,
		yOffset = 4885.574573158966,
		zoneData = {
			ScarletEnclave = {
				height = 1333.510259086504,
				width = 2000.248684963022,
				xOffset = 22459.46800337001,
				yOffset = 7672.207074316265,
			},
		}
	}
}

-- move zoneData keys into zone-number-indexed entries.
local zeroData;
